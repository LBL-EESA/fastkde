...
class fastKDE:
  ...
  def __init__( self,\
    ...
    ------
      ...
      log_axes             : Flags whether axes should be log spaced (i.e., the PDF is calculated
                            ...
                            for each variable (i.e, len(log_axes) == shape(data)[0]) specifying which
                            axes should use log spacing.  If only True or False is given, that value
                            is used for all variables.

      num_points_per_sigma   : the number of points on the data grid per standard
      ...
      num_points           : the number of points to use for the pdf grid. If provided as a scalar,
      ...
      do_approximate_ecf    : flags whether to approximate the ECF using a (much faster)
                            FFT.  In tests, this is accurate to ~1e-14 over low 
                            frequencies, but is inaccurate to ~1e-2 for the highest ~5% 
                            of frequencies.

      ecf_precision        : sets the precision of the approximate ECF.  If set to 2, it uses
                            double precision accuracy; 1 otherwise

      do_fft               : flags whether to calculate phiSC and its FFT to obtain
                            pdf

      do_save_marginals     : flags whether to calculate and save the marginal distributions

      frac_contiguous_hyper_volumes :  the fraction of contiguous hypervolumes of the ECF, that 
                                    are above the ECF threshold, to use in the density estimate

      num_contiguous_hyper_volumes : like frac_contiguous_hyper_volumes, but specify an integer number
                                  to use.  frac_contiguous_hyper_volumes will be ignored if this
                                  is provided as an argument.

      positive_shift     : translate the PDF vertically such that the estimate is positive or
                          0 everywhere

      count_threshold    : this argument does nothing; it has been deprecated.  It is kept as an argument for backward
                          compatibility.

      axis_expansion_factor : sets the amount by which the KDE grid will be expanded relative to the original min-max
    ...
    if(data is not None):
      ...
      if(data_rank > 2):
          raise ValueError("data must be a rank-2 array of shape [num_variables,num_data_points]")
      ...
      for v in range(self.num_variables):
          ...
          if log_axes[v]:
              ...
              if(npy.amin(data[v,:]) <= 0):
                  raise ValueError("logarithmic axes were specified for variable {}, but that variable contains values less than 0: min = {}".format(v,npy.amin(data[v,:])))
      ...
      vprint("Operating on data with num_variables = {}, num_data_points = {}".format(self.num_variables,self.num_data_points))
    ...
    if(axes is None):
        ...
        #inflate the range by axis_expansion_factor to ensure that KDE power doesn't leak through
        ...
        if num_points is None:
            ...
            self.numXPoints = npy.array([next_highest_power_of_two(ns * num_points_per_sigma) + int(addOne) for ns in numSigma])
        else:
            ...
            if isIterable:
                if lenNum == self.num_variables:
                    ...
                    raise ValueError("len(num_points) = {}, but it should match num_variables = {}".format(lenNum,self.num_variables))
            else:
                self.numXPoints = npy.array(self.num_variables*(num_points,))


        #Set the grids for each dimension
        self.axes = [ npy.linspace(xmin,xmax,np) for xmin,xmax,np in zip(self.xMin,self.xMax,self.numXPoints)]
    ...
    self.dataNorm = (self.xMax - self.xMin)/(2.*npy.pi)
    ...
    self.tgrids = [ calc_t_from_x((xg-av)/sd) for xg,av,sd in zip(self.axes,self.dataMid,self.dataNorm) ]
    ...
    if(data is not None):
      ...
      ecfObj = ecf.ECF( input_data = data, \
                        tgrids = self.tgrids, \
      ...
      if(self.do_fft):
        ...
        if(self.do_save_marginals):
          ...
          for i in range(self.num_variables):
            self.marginalObjects.append(fastKDE(original_data[i,:], \
  ...
  def applyBernacchiaFilter(self,doFlushArrays=True):
    ...
    contiguousInds = flood.flood_fill_search(ecfSq,search_threshold = self.ecfThreshold)

    if contiguousInds == []:
        raise RuntimeError("No ECF values found above the ECF threshold.  max(ecfSq) = {}, ecfThresh = {}".format(npy.amax(ecfSq),ecfThresh))

    #Sort them by distance from the center
    sortedInds = flood.sort_by_distance_from_center(contiguousInds,npy.shape(ecfSq))
    ...
    #Pull out frac_contiguous_hyper_volumes of contiguous hyper volumes, in order of distance from
    #the origin
  ...
  def __transformphiSC__(self):
    ...
    pdf = npy.fft.fftshift(npy.real(npy.fft.fftn(npy.fft.ifftshift(self.phiSC))))*npy.prod(self.deltaT)*(1./(2*npy.pi))**self.num_variables
    ...
    if self.positive_shift:
        if len(npy.where(self.pdf < 0)[0]) != 0:
            ...
            if not npy.isfinite(delta) or delta < 0 or delta >= npy.amax(self.pdf):
                if self.be_verbose:
                    print('positive_shift algorithm failure: defaulting to no shift')
                delta = 0.0
    ...
    if(self.do_save_transformed_kernel):
      kSC = npy.fft.fftshift(npy.real(npy.fft.fftn(npy.fft.ifftshift(self.kappaSC))))*npy.prod(self.deltaT)*(1./(2*npy.pi))**self.num_variables
  ...
  def __transformphiSC_points__(self,list_of_points):
    ...
    # transform the PDF estimate to real space
    #pdf = dft_points(array(self.tgrids,dtype=float),self.phiSC,list_of_points)*prod(self.deltaT)*(1./(2*pi))**self.num_variables
    ...
    pdf = dft_points(frequencyGrids,
                     ...
                     missingFreqVal = fillValue) * \
                             npy.prod(self.deltaT)*(1./(2*npy.pi))**self.num_variables
  ...
  def estimateConditionals(self,variables,data,peakFrac = 0.0,reApplyFilter=False):
      ...
      marginalPDF = npy.ma.masked_less_equal(marginalObject.pdf[conformantSlice],marginalThreshold)
      ...
      if(reApplyFilter):
          conditionalPDF = npy.ma.masked_less_equal(self.reApplyFilter(conditionalPDF),0.0)
      ...
      normFactor = npy.ma.masked_equal(sum(conditionalPDF*dxProd,axis=tuple(sumAxes)),0.0)
  ...
  def getCopula(self,data=None,peakFrac = 0.0):
    ...
    if(not self.do_save_marginals):
      if(data is None):
        raise ValueError("the data must be provided as argument 'data', if do_save_marginals=False when the original PDF was calculated")
      else:
  ...
  def reApplyFilter(self,pdf):
      ...
      phiTilde_tmp = npy.fft.fftshift(npy.fft.ifftn(npy.fft.ifftshift(npy.ma.filled(pdf,0.0))))
      ...
      #Transform the PDF estimate to real space
      pdf = npy.fft.fftshift(npy.real(npy.fft.fftn(npy.fft.ifftshift(phiTilde))))*npy.prod(self.deltaT)*(1./(2*npy.pi))**self.num_variables
  ...
  def __add__(self,rhs):
    ...
    retObj.ECF = (self.num_data_points*self.ECF + rhs.num_data_points*rhs.ECF) \
...
def pdf(*args,**kwargs):
    ...
    try:
        do_save_marginals = kwargs['do_save_marginals']
...
def conditional( \
    ...
    cpdf = _pdf.estimateConditionals(cvarInds,npy.array(fullVarList),peakFrac = peakFrac)
...
def pdf_at_points(*args,**kwargs):
    ...
    if list_of_points_provided_in_kwargs:
        ...
        if(data_rank > 2):
            ...
            raise ValueError("list_of_points must be able to be broadcast to a rank-2 array of shape [num_data_points,num_variables]")
    ...
    try:
        _ = kwargs['log_axes']
        del(kwargs['log_axes'])
        warnings.warn("fastKDE.pdf_at_points() does not currently support the log_axes option; it will be ignored.")
    except:
        pass
    ...
    try:
        ...
        del(kwargs['positive_shift'])
        warnings.warn("fastKDE.pdf_at_points() does not currently support the positive_shift option; it will be ignored.")
    except:
        pass
...
if(__name__ == "__main__"):
  ...
  if(doOneDimensionalTests):
    ...
    def mygaus(x):
      return (1./(sig*npy.sqrt(2*npy.pi)))*npy.exp(-(x-mu)**2/(2.*sig**2))
    ...
    if evaluateError:
        ...
        for i,n in zip(range(len(npow)),npow):
          ...
          with Timer(nsample[i]):
            ...
            bkernel = fastKDE(randgauss,do_approximate_ecf=True,num_points=513)
          ...
          esq[i] = npy.average(abs(mygaus(bkernel.axes[0])-bkernel.pdf[:])**2 *bkernel.deltaX[0])
        ...
        if(not bDemoSum):
          ...
          for i in range(nloop):
            ...
            #Calculate the mean squared error between the estimated density
            #And the gaussian
            esq2[i] = npy.average(npy.abs(mygaus(bkernel2.axes[0])-bkernel2.pdf)**2 * bkernel2.deltaX[0])
            #Print the sample size and the error to show that the code is proceeeding
  ...
  if(doTwoDimensionalTests):
    ...
    def norm2d(x,y,mux=0,muy=0,sx=1,sy=1,r=0):
      coef = 1./(2*npy.pi*sx*sy*npy.sqrt(1.-r**2))
    ...
    for gg in gausParams:
      ...
      randsamples.append(npy.random.multivariate_normal(mu,gCovMat,(nmax/ngg,)).transpose())
    ...
    if(doPlot):
      ...
      clevs = npy.asarray(range(2,10))/100.
