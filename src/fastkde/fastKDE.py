...
class fastKDE:
  ...
  def __init__( self,\
    ...
    if(axes is None):
        ...
        vprint(\
            (
             "Grids created with xmin: {}, xmax: {}, " + 
             "npoints: {}"
            ).format(self.xMin,self.xMax,self.numXPoints)
            )
    else:
    ...
    for v in range(self.num_variables):
        ...
        dx = (xg[1:]-self.dataMid[v])/self.dataNorm[v] - \
             (xg[:-1] - self.dataMid[v])/self.dataNorm[v]
        ...
        if log2size != npy.floor(log2size):
            ...
            raise ValueError(\
                 "All grids in axes must be powers of 2" + extraStr + 
  ...
  def applyBernacchiaFilter(self,doFlushArrays=True):
    ...
    if(self.be_verbose):
        print(
            (
             "Normalization of kappaSC, ECF, and phiSC: "+
             "{}, {}, {}"
            ).format(\
  ...
  def __transformphiSC__(self):
    ...
    pdf = npy.fft.fftshift(
            npy.real(npy.fft.fftn(npy.fft.ifftshift(self.phiSC)))) \
    ...
    if(self.be_verbose):
      ...
      print(\
          (
           ...
           "phiSC[0] = {}"
          ).format(normConst,self.phiSC[midPointAccessor])
          )
  ...
  def estimateConditionals(self,variables,data,peakFrac = 0.0,reApplyFilter=False):
      ...
      if len(rightSideVariableIndices) > self.num_variables:
          raise ValueError(
              "More indices were provided in 'variables' than there are "
              "variables."
              )
  ...
  def reApplyFilter(self,pdf):
      ...
      # Transform the PDF estimate to real space
      pdf = npy.fft.fftshift(\
              npy.real(npy.fft.fftn(npy.fft.ifftshift(phiTilde)))) \
              *npy.prod(self.deltaT)*(1./(2*npy.pi))**self.num_variables
    ...
    for key in varKeys:
        ...
        if key != "var1":
            try:
                ...
                raise ValueError(\
                    "Incomprehensible variable-like keyword provided: "
                    "{}".format(key)
                    )
...
if(__name__ == "__main__"):
  ...
  if(doOneDimensionalTests):
    ...
    if evaluateError:
        ...
        for i,n in zip(range(len(npow)),npow):
          ...
          # *bkernel.deltaX)
          esq[i] = npy.average(\
              abs(mygaus(bkernel.axes[0])-bkernel.pdf[:])**2 *bkernel.deltaX[0]
              )
          epct[i] = 100*sum(
        ...
        if(not bDemoSum):
          ...
          for i in range(nloop):
            ...
            esq2[i] = npy.average(\
                npy.abs(\
                    mygaus(bkernel2.axes[0])-bkernel2.pdf)**2 \
                        * bkernel2.deltaX[0]
                )
          ...
          plt.subplot(2,2,4)
          plt.plot(bkernel2.axes[0], abs(bkernel.pdf - bkernel2.pdf)*bkernel.deltaX[0])
  ...
  if(doTwoDimensionalTests):
    ...
    def norm2d(x,y,mux=0,muy=0,sx=1,sy=1,r=0):
      ...
      expArg = -(1./(2*(1-r**2)))* \
          ( (x-mux)**2/sx**2 + (y-muy)**2/sy**2 - 2*r*(x-mux)*(y-muy)/(sx*sy))
