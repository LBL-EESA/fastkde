...
def stochasticSample(x0=305,y0=200,yrange=4,numSamples=1e3, \
    ...
    y = underlyingFunction(x,x0,y0,yrange) + npy.random.normal(loc=yp1,scale=yp2,size=numSamples)
...
def getModeCurve(conditional,axes):
    ...
    modeCurve = npy.array([ axes[1][conditional[:,i].argmax()] for i in range(conditional.shape[1])])
...
def asciiToPoints(text,convertCharacter = " "):
    ...
    pointArray = npy.array([ [ 1 if char == convertCharacter else 0 for char in line] for line in textLines]).T[:,::-1]
...
def genCovMat(varianceList,correlationDict):
    ...
    for i,j in pairList:
        covMat[i,j] = covMat[j,i] = correlationDict[(i,j)]*npy.sqrt(varianceList[i])*npy.sqrt(varianceList[j])
...
def estimatePDFWrapper(arg,**kwargs):
    ...
    for numSamples in sampleList:
        ...
        if conditionVar is None:
            ISE = npy.ma.sum(deltaXProducts*(pdfTestObj.pdfStandard(_pdf.axes) - _pdf.pdf)**2)
        else:
            ISE = npy.ma.sum(deltaXProducts*(pdfTestObj.pdfStandard(_pdf.axes) - cpdf)**2)/len(cpdf.compressed())
...
class testNormal1D(testDistribution):
    ...
    def pdfStandard(self,axes):
        ...
        sig = self.sig
        mu = self.mu
        pdfVal = (1./(sig*npy.sqrt(2*npy.pi)))*npy.exp(-(x-mu)**2/(2.*sig**2))
...
class testNormalND(testDistribution):
    ...
    def pdfStandard(self,axes):
        ...
        xarrays = npy.transpose(npy.array(npy.meshgrid(*axes,indexing='ij')),axes=lastFirst)
